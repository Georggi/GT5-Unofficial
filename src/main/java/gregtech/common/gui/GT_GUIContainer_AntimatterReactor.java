...
public class GT_GUIContainer_AntimatterReactor extends GT_GUIContainerMetaTile_Machine {

    public String mNEI;
    String mName = "";

    public GT_GUIContainer_AntimatterReactor(InventoryPlayer aInventoryPlayer, IGregTechTileEntity aTileEntity, String aName, String aTextureFile, String aNEI) {
    ...
    protected void drawGuiContainerForegroundLayer(int par1, int par2) {
        ...
        if (mContainer != null) {
            ...
            if (this.mContainer != null) {
            	if ((this.mContainer.mProgressTime/20) <= 60){
            		fontRendererObj.drawString(GT_Utility.formatNumbers((this.mContainer.mProgressTime)/20)+ " sec" + "/" + GT_Utility.formatNumbers(((this.mContainer.mMaxProgressTime)/20)/60) + " min", 50, 155, 0x00ff0000);
            	} else {
            		fontRendererObj.drawString(GT_Utility.formatNumbers(((this.mContainer.mProgressTime)/20)/60)+ " min" + "/" + GT_Utility.formatNumbers(((this.mContainer.mMaxProgressTime)/20)/60) + " min", 43, 155, 0x00ff0000);
            	}
            }
        }
    }
